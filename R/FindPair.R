...
#' @export
#'
#' @examples FindStabRvsPair(ControlFrame,TreatFrame,HighPercent=0.9,LowPercent=0.1)
FindStabRvsPair<-function(ControlFrame,TreatFrame,HighPercent=0.9,LowPercent=0.1,ncore=1){
  ...
  if (ncore==1) {
    ...
    fun1<-function(x){
      for(i in x){
        ...
        cat(paste0(Sys.time()," ","Control:There are ",(nrow(ControlFrame)-(i+1))," data left unprocessed."))
        return(ControlData)
      }
    }
    ...
    res <- parLapply(cl,1:(nrow(ControlFrame)-1), fun=fun1)
    stopCluster(cl)
    ...
    fun2<-function(x){
      for(i in x){
        ...
        cat(paste0(Sys.time()," ","Treat:There are ",(nrow(TreatFrame)-(i+1))," data left unprocessed."))
        return(TreatData)
      }
    }
    ...
    clusterExport(cl,"HighPercent",envir = environment())
    clusterExport(cl,"LowPercent",envir = environment())
    res <- parLapply(cl,1:(nrow(TreatFrame)-1), fun=fun2)
    stopCluster(cl)
